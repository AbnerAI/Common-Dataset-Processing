...
# /media/oem/sda21/cxx/breast_all/Video

output_path = '/media/oem/sda21/cxx/breast_all/Video/All_C_Train_Crop'
lists = os.listdir('/media/oem/sda21/cxx/breast_all/Video/All_C_Train')
index_ = 0
error_f = open('./error_c_zk.txt','a')
for item in lists:
    index_ += 1
    if not os.path.exists(output_path + '/' + item):
        os.makedirs(output_path + '/' + item)
    ids = glob.glob('/media/oem/sda21/cxx/breast_all/Video/All_C_Train/'+ item + '/' +'*.jpg')
