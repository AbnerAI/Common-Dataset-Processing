...
def main():
    ...
    output = '/home/qianxianserver/data/cxx/breast_all/breast/视频-2021-2/2021_breast_three_classification_zk1/Mix_Data/New/Test'
    path = output

    if not os.path.exists(output):
        os.mkdir(output)
    
    txtName = './mix_lists_test_breast_all.txt'
    f=open(txtName, "a+")

    file_list_c = os.listdir(path + '/C') #.代表根目录下
    file_list_n = os.listdir(path + '/N') #.代表根目录下
    file_list_h = os.listdir(path + '/H') #.代表根目录下
    ...
    for item_h in file_list_h:
        print(path + '/H/' +item_h)
        if len(os.listdir(path + '/H/' +item_h))!=0:
            listing = os.listdir(path + '/H/' +item_h)
            for files in listing:
                if files.endswith('.jpg'):
                    f.write(path + '/H/' +item_h + ' H' + '\n')
                    break
